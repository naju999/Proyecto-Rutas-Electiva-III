...
function isTileRequest(request) {
  ...
  return sameProtocol && hostMatch && (looksLikeTile || tileExtension) && isTunjaTileRequest(url);
}

function isTunjaTileRequest(url) {
  const match = url.pathname.match(/\/(\d+)\/(\d+)\/(\d+)\.(png|jpg|jpeg|webp)$/i);
  ...
  const latRadians = Math.atan(Math.sinh(Math.PI * (1 - (2 * y) / n)));
  const lat = (latRadians * 180) / Math.PI;

  return (
    lat >= TUNJA_BOUNDS.minLat &&
    lat <= TUNJA_BOUNDS.maxLat &&
    lng >= TUNJA_BOUNDS.minLng &&
    lng <= TUNJA_BOUNDS.maxLng
  );
}
...
async function matchAssetFromCaches(request) {
  const cacheNames = [APP_SHELL_CACHE_NAME, FAVORITE_ROUTE_CACHE_NAME];
...
async function handleAppAssetRequest(request, event) {
  const url = new URL(request.url);
  const isDataAsset = url.pathname.startsWith('/data/');
  const appCache = await caches.open(APP_SHELL_CACHE_NAME);
  const cachedResponse = await matchAssetFromCaches(request);
  ...
  try {
    ...
    if (networkResponse && networkResponse.ok) {
      if (!isDataAsset) {
        const targetCache = await caches.open(APP_SHELL_CACHE_NAME);

        try {
          await targetCache.put(request, networkResponse.clone());
        } catch (_cacheError) {
          // Si el caché del shell está lleno, se entrega la respuesta de red sin bloquear la UI.
        }
      }
    }
...
async function handleTileRequest(event) {
  ...
  if (cachedResponse) {
    event.waitUntil((async () => {
      await touchTileMetadata(request.url);
